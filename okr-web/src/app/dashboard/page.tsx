...
export default function Dashboard() {
  ...
  const { 
    ...
    error: summaryError 
  } = useCachedAPI<DashboardSummary>(`/dashboard/summary?quarter=${quarter}`, {
  ...
  } = useCachedAPI<TopPerformer[]>(`/dashboard/top-performers?quarter=${quarter}`, {
    ttl: 5 * 60 * 1000 // 5 minutes cache
  });

  const [recentCheckIns] = useState<RecentCheckIn[]>([]);
  const loading = summaryLoading || performersLoading;
        ...
        {summary && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
              ...
              <p className="text-2xl font-bold text-gray-900">{summary.total_objectives || 0}</p>
            </div>
            
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
              <h3 className="text-sm font-medium text-gray-500 mb-2">Average Progress</h3>
              <p className="text-2xl font-bold text-gray-900">
                {summary.average_progress ? summary.average_progress.toFixed(1) : 0}%
              </p>
            </div>
            ...
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
              ...
              <p className="text-2xl font-bold text-green-600">
                {summary.status_distribution?.on_track || 0}
              </p>
            </div>
          </div>
        )}
